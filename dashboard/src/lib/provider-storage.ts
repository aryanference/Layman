...
export const PROVIDERS_STORAGE_KEY = "meridian-providers-v2";
export const CHAT_PROVIDER_SELECTION_KEY = "meridian-chat-provider-selection-v2"
...
export function loadChatProviderSelection(): ChatProviderSelection {
  ...
  try {
    ...
    return {
      ...
      modelId: parsed.modelId ?? "google/gemma-2-9b-it:free",
    };
  } catch {
    return {
      providerId: "openrouter",
...
export function getActiveCustomProviders(): ProviderConfig[] {
  const all = loadProviderConfigs();
  return all.filter((p) => p.id !== "openrouter" && p.apiKey.trim().length > 0 && p.enabled);
}
