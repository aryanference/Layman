    ...
    if (match[2] && match[3]) {
      nodes.push(
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="var(--text-hero) underline decoration-white/30 underline-offset-4 transition hover:decoration-white/60"
        >
          {match[2]}
        </a>,
      );
      ...
      nodes.push(
        <code
          key={key}
          className="rounded-md rgba(255,255,255,0.08) px-1.5 py-0.5 font-mono text-[0.92em] var(--text-hero)"
        >
          {match[4]}
        </code>,
      );
      ...
      nodes.push(
        <strong key={key} className="font-semibold var(--text-hero)">
      ...
      nodes.push(
        <em key={key} className="italic var(--text-primary)">
...
export default function MarkdownPreview({
  ...
  const paragraphClass = compact ? "text-sm var(--text-primary)" : "text-sm var(--text-primary)";
  const listClass = compact ? "ml-5 space-y-1 text-sm var(--text-primary)" : "ml-5 space-y-1.5 text-sm var(--text-primary)";
    ...
    <div className={joinClasses("min-w-0 break-words", gapClass, className)}>
      {blocks.map((block, index) => {
        ...
        if (block.type === "heading") {
          const headingClass =
            block.depth <= 2
              ? "text-base font-semibold var(--text-hero)"
              : "text-sm font-semibold uppercase tracking-[0.14em] var(--text-secondary)";
        ...
        if (block.type === "quote") {
          return (
            <blockquote
              key={key}
              className="border-l-2 var(--glass-border) pl-4 text-sm italic var(--text-secondary)"
            >
              {renderInlineContent(block.content, key)}
            </blockquote>
          );
        ...
        if (block.type === "code") {
          return (
            <div
              key={key}
              className="overflow-x-auto rounded-2xl border var(--glass-border) rgba(255,255,255,0.06) px-4 py-3"
            >
              {block.language ? (
                <p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.18em] var(--text-muted)">
                  {block.language}
                </p>
              ) : null}
              <pre className="whitespace-pre-wrap font-mono text-[13px] leading-6 var(--text-primary)">
                {block.content}
              </pre>
            </div>
          );
        ...
        return <div key={key} style={{ height: "1px", background: "var(--glass-border)" }} />;
      })}
    </div>
  );
}
