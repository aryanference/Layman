...
async def _call_openai_compatible(
  *,
  ...
  try:
    ...
    raise RuntimeError(
      "model_timeout: The model did not respond in time. Try mistral-7b or llama-3-8b instead."
    )
  ...
  if response.status_code != 200:
    ...
    raise RuntimeError(
      f"model_error (status {response.status_code}): "
      f"{(error_detail or 'No detail available').strip()}. "
      "This model may be unavailable. Try mistral-7b or llama-3-8b instead."
    )
